...
MemPool::MemPool(FramePool * _frame_pool, int _n_frames) {
  ...
  start_address = _frame_pool->get_frame() * PAGE_SIZE;
  for (int i = 1; i < _n_frames; i++) {
      unsigned long next_frame_addr = _frame_pool->get_frame() * PAGE_SIZE;
  }
  Console::puts("done\n");
}     
...
unsigned long MemPool::allocate(unsigned long _size) {
  ...
  start_address = start_address + (_size / PAGE_SIZE + 1) * PAGE_SIZE;
